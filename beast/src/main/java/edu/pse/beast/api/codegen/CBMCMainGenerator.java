...
public class CBMCMainGenerator {
	...
	public static CFunction main(BooleanExpASTData preAstData, BooleanExpASTData postAstData, ElectionTypeCStruct voteArrStruct,
		...
		String lower = "unsigned int VAR = 0;".replaceAll("VAR", votesLowerBoundVarName);
		String upper = "unsigned int VAR = C;".replaceAll("VAR", votesUpperBoundVarName);
		...
		for(int i = 0; i < highestVote; ++i) {
			String varName = "voteNUMBER".replaceAll("NUMBER", String.valueOf(i + 1)); 
			code.add("//initializing VAR".replaceAll("VAR", varName));
			code.add(InitVoteHelper.generateCode(varName, voteArrStruct, options));
		}		
		
		CodeGenASTVisitor visitor = new CodeGenASTVisitor(voteArrStruct, voteResultStruct, options);
