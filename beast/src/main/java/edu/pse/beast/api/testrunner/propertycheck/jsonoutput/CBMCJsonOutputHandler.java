...
public class CBMCJsonOutputHandler {
	...
	public CBMCJsonOutputHandler(CElectionDescription descr, PreAndPostConditionsDescription propDescr,
			CBMCGeneratedCodeInfo cbmcGeneratedCodeInfo, int s, int c, int v, List<String> rawOutput) {
		this.descr = descr;
	...
	public String getExampleText() {
		...
		for (String varName : varNameToOtherVoteAssignments.keySet()) {
			...
			if (cbmcGeneratedCodeInfo.hasInfo(varName)) {
				completeString += cbmcGeneratedCodeInfo.getInfo(varName).getInfo() + "\n";
			}
		...
		for (Integer electNumber : electNumberToAssignmentString.keySet()) {
			...
			for (String memberName : memberAssignments.keySet()) {
				list.add("    " + memberName + " = " + memberAssignments.get(memberName));
			}
			list.sort((s1, s2) -> {
				return s1.compareTo(s2);
			});
	...
	private void processCBMCJsonOutput(CBMCGeneratedCodeInfo cbmcGeneratedCodeInfo) {
		...
		String jsonString = "{ " + OUTPUT_KEY + " : " + String.join("\n", rawOutput) + "}";
		...
		for (int i = 0; i < traceArr.length(); ++i) {
			...
			if (traceJsonObj.getString(STEP_TYPE_KEY).equals(STEP_TYPE_VALUE_ASSIGNMENT)) {
				JSONObject valueJsonObj = traceJsonObj.getJSONObject(ASSIGNMENT_VALUE_KEY);
				if (!traceJsonObj.has("sourceLocation")) {
					continue;
				}
				JSONObject locationJsonObj = traceJsonObj.getJSONObject("sourceLocation");
