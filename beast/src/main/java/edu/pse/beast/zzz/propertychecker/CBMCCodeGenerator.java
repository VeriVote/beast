...
import static edu.pse.beast.zzz.toolbox.CCodeHelper.arr;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.arrAcc;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.arrAccess;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.conjunct;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.define;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.defineIfNonDef;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.dot;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.dotArrStructAccess;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.eq;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.forLoopHeaderCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.functionCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.i;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.include;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.intVar;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.intVarEqualsCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.j;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.leq;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.lineBreak;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.lineComment;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.lt;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.neq;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.one;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.parenthesize;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.pointer;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.space;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.spaces;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.two;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.typeVar;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.uintVarEqualsCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.unsignedIntVar;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.varAddCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.varAssignCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.varDivideCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.varMultiplyCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.varSubtractCode;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.x;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.y;
import static edu.pse.beast.zzz.toolbox.CCodeHelper.zero;
...
import edu.pse.beast.zzz.electionsimulator.ElectionSimulationData;
 ...
 *
 * <p>
 * TODO Refactor this into multiple sub classes later on.
 *
...
public class CBMCCodeGenerator {
	...
	private static final String COMMENT_EMPTY_SUB_ARRAY = "The sub array should be empty.";
	/** The Constant COMMENT_START_STOP. */
	private static final String COMMENT_START_STOP = "Start is inclusive, stop is exclusive.";
	/** The Constant COMMENT_LIMIT_UPPER_BOUND. */
	private static final String COMMENT_LIMIT_UPPER_BOUND = "Limit the size to the upper bound V.";
	/** The Constant COMMENT_SET_BEGINNING. */
	private static final String COMMENT_SET_BEGINNING = "Set all to C in the beginning.";
	...
	/**
	 * Loop bounds for header functions for pre- and postconditions, saved as pairs
	 * of (unique) loop name and bound value.
	 */
	 ...
	 *
	 * @param electionDescription the election decription that holds the code that
	 *                            describes the voting method. that code will be
	 *                            merged with the generated code
	 * @param propCondDesc        the Descriptions that will be used to generate the
	 *                            C-Code for CBMC
	 */
	public CBMCCodeGenerator(final ElectionDescription electionDescription,
		...
		this.visitor = new CBMCCodeGenerationVisitor(electionDescription.getContainer());
		// new CCodeHelper();
		generateCodeCheck();
	}
	 ...
	 *                            describes the voting method. that code will be
	 *                            merged with the generated code
	 * @param propertyCondDesc    the descriptions that will be used to generate the
	 *                            C-Code for CBMC
	 * @param marginVal           the margin
	 * @param origResultData      the original election result
	 * @param votingData          the voting data
	 */
	public CBMCCodeGenerator(final ElectionDescription electionDescription,
			final PreAndPostConditionsDescription propertyCondDesc, final int marginVal,
			final ElectionSimulationData origResultData, final ElectionSimulationData votingData) {
		this.translator = new FormalPropertySyntaxTreeToAstTranslator();
		this.electionDesc = electionDescription;
	 ...
	 */
	public CBMCCodeGenerator(final ElectionDescription electionDescription,
			final PreAndPostConditionsDescription propertyCondDesc, final ElectionSimulationData votingData) {
		this.translator = new FormalPropertySyntaxTreeToAstTranslator();
		this.electionDesc = electionDescription;
		this.preAndPostCondDesc = propertyCondDesc;
		code = new CodeArrayListBeautifier();
	 ...
	 *
	 * @return the loop bounds for headers with a mapping from unique loop
	 *         identifiers.
	 */
	...
	private void generateCodeMargin(final ElectionSimulationData votingData) {
		...
		final String origVotes = varAssignCode(
				electionDesc.getContainer().getInputStruct().getStructAccess() + space()
						+ UnifiedNameContainer.getOrigVotesName(),
				getVotingResultCode((CBMCResultValueWrapper) votingData.getValues())) + CCodeHelper.SEMICOLON;
		final String sizeOfVote = "" + electionDesc.getContainer().getInputType()
				.getSizesInOrder(votingData.getVoters(), votingData.getCandidates(), votingData.getSeats()).get(0);
		final String origVotesSize = uintVarEqualsCode(ORIG_VOTES_SIZE) + sizeOfVote + CCodeHelper.SEMICOLON;
	...
	private void generateCodeTest(final ElectionSimulationData votingData) {
		...
		addMarginHeaders(votingData);
		// Add the code the user wrote (e.g the election function).
		//NICE!!!!! USING THE GUI HERE!!! AWESOME!!! GREAT
		final ElectionDescription descr = GUIController.getController().getElectionDescription();
		code.addAll(descr.getComplexCodeAndSetHeaderLoopBounds());
		headerLoopBounds.putAll(descr.getLoopBounds());
		// Add the code which defines the votes.
	 ...
	 */
	private void addMarginMainCheck(final int marginVal, final ElectionSimulationData origData) {
		...
		// Add the main method.
		code.add(intVar(functionCode(MAIN)) + space() + CCodeHelper.OPENING_BRACES);
	 ...
	 */
	private void addMarginCompMethod(final CodeArrayListBeautifier codeList, final InputType inType,
			final OutputType outType, final String origResultName) {
	 ...
	 */
	private void addMarginCompMethodInput(final CodeArrayListBeautifier codeList, final InputType inType,
			final String voteName) {
		codeList.add(intVarEqualsCode(TOTAL_DIFF) + zero() + CCodeHelper.SEMICOLON);
		codeList.add(intVarEqualsCode(POS_DIFF) + zero() + CCodeHelper.SEMICOLON);
		final String voteContainer = inType.getStruct().getStructAccess() + space() + voteName + CCodeHelper.SEMICOLON;
		...
		final List<String> loopVars = addNestedForLoopTop(codeList, inType.getSizeOfDimensionsAsList(),
				new ArrayList<String>());
		inType.flipVote(voteName, UnifiedNameContainer.getOrigVotesName(), loopVars, codeList);
		...
		codeList.add(functionCode(ASSUME, leq(POS_DIFF, MARGIN)) + CCodeHelper.SEMICOLON);
		codeList.add(functionCode(ASSUME, eq(TOTAL_DIFF, zero())) + CCodeHelper.SEMICOLON);
	}
	 ...
	 * @param origResultName the orig result name
	 */
	private void addMarginCompMethodOutput(final CodeArrayListBeautifier codeList, final OutputType outType,
			final String newVotesName, final String origResultName) {
		final String resultName = UnifiedNameContainer.getNewResultName();
		final String resultContainer = outType.getStruct().getStructAccess() + space() + resultName;
		final String resultAssignment = varAssignCode(resultContainer,
				functionCode(UnifiedNameContainer.getVotingMethod(), ORIG_VOTES_SIZE, newVotesName))
				+ CCodeHelper.SEMICOLON;
		codeList.add(resultAssignment);
		final List<String> loopVars = addNestedForLoopTop(codeList, outType.getSizeOfDimensionsAsList(),
				new ArrayList<String>());
		...
		codeList.add(functionCode(ASSERT, eq(newResultAcc, origResultAcc)) + CCodeHelper.SEMICOLON);
	 ...
	 * @return the list
	 */
	public static List<String> addNestedForLoopTop(final CodeArrayListBeautifier code, final List<String> dimensions,
	 ...
	 */
	public static void addNestedForLoopBot(final CodeArrayListBeautifier code, final int dimensions) {
	...
	private void addMarginHeaders(final ElectionSimulationData votingData) {
		code = addHeader(code);
		code.add(defineIfNonDef(UnifiedNameContainer.getVoter(), votingData.getVoters()));
		code.add(defineIfNonDef(UnifiedNameContainer.getCandidate(), votingData.getCandidates()));
		code.add(defineIfNonDef(UnifiedNameContainer.getSeats(), votingData.getSeats()));
	}
	...
	private void addVoteSumFunc(final boolean unique) {
		...
		final String input = electionDesc.getContainer().getInputStruct().getStructAccess() + space() + TMP_STRUCT;
		code.add(unsignedIntVar(functionCode(funcName, input, unsignedIntVar(AMOUNT_VOTES), unsignedIntVar(CANDIDATE)))
				+ space() + CCodeHelper.OPENING_BRACES);
		final int dimensions = electionDesc.getContainer().getInputType().getAmountOfDimensions();

		final String[] sizes = electionDesc.getContainer().getInputType().getSizeOfDimensions();
		...
		final String dataDef = electionDesc.getContainer().getInputType().getDataTypeAndSign();
		final String definition = dataDef + space() + arr()
				+ electionDesc.getContainer().getInputType().getDimensionDescriptor(true) + CCodeHelper.SEMICOLON;
		code.add(definition);
		final String assignment = forLoopStart
				+ varAssignCode(arrAccess(arr(), access), dotArrStructAccess(TMP_STRUCT, access))
				+ CCodeHelper.SEMICOLON + lineBreak(forLoopEnd);
		code.add(assignment);
	...
	private void addIntersectFunction() {
		...
		code.add(electionDesc.getContainer().getOutputStruct().getStructAccess() + space()
				+ functionCode(INTERSECT,
						electionDesc.getContainer().getOutputStruct().getStructAccess() + space() + VOTES + one(),
						electionDesc.getContainer().getOutputStruct().getStructAccess() + space() + VOTES + two())
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(electionDesc.getContainer().getOutputStruct().getStructAccess() + space() + TO_RETURN
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), C));
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(dotArrStructAccess(TO_RETURN, i()), zero())
				+ CCodeHelper.SEMICOLON);
		...
		code.add(spaces(SPACES_PER_TAB)
				+ functionCode(CCodeHelper.IF,
						eq(dotArrStructAccess(VOTES + one(), i()), dotArrStructAccess(VOTES + two(), i())))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
				+ varAssignCode(dotArrStructAccess(TO_RETURN, i()), dotArrStructAccess(VOTES + one(), i()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.RETURN + space() + TO_RETURN + CCodeHelper.SEMICOLON);
		code.add(CCodeHelper.CLOSING_BRACES);
	}
	...
	private void addPermutateTwoFunction() {
		if (electionDesc.getContainer().getInputType().getAmountOfDimensions() != SPACES_PER_HALF_TAB
		...
		final String voteStruct = electionDesc.getContainer().getInputStruct().getStructAccess();

		code.add(voteStruct + space() + functionCode(PERMUTATE_TWO, typeVar(voteStruct, VOTES), unsignedIntVar(LENGTH))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(voteStruct + space() + SUB_ARR + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(unsignedIntVar(arrAccess(ALREADY_USED_ARR, V)) + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(forLoopHeaderCode(i(), lt(), V) + space() + lineComment(COMMENT_SET_BEGINNING));
		code.add(forLoopHeaderCode(j(), lt(), C));
		code.add(varAssignCode(dotArrStructAccess(SUB_ARR, i(), j()), C) + CCodeHelper.SEMICOLON);
		code.add(CCodeHelper.CLOSING_BRACES);
		code.add(CCodeHelper.CLOSING_BRACES);
		code.add();
		code.addSpaces(SPACES_PER_HALF_TAB);
		...
		code.add(varAssignCode(unsignedIntVar(NEW_INDEX), functionCode(NONDET_UINT)) + CCodeHelper.SEMICOLON);
		code.add(functionCode(ASSUME, conjunct(leq(zero(), NEW_INDEX), lt(NEW_INDEX, LENGTH))) + CCodeHelper.SEMICOLON);
		code.add();
		code.add(forLoopHeaderCode(j(), lt(), i()));
		addVoteLoopBound(PERMUTATE_TWO + ".4");
		code.add(functionCode(ASSUME, neq(NEW_INDEX, arrAccess(ALREADY_USED_ARR, j()))) + CCodeHelper.SEMICOLON);
		code.add(CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(arrAccess(ALREADY_USED_ARR, i()), NEW_INDEX)
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
				+ varAssignCode(dotArrStructAccess(SUB_ARR, NEW_INDEX, j()), dotArrStructAccess(VOTES, i(), j()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		...
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
				+ varAssignCode(dotArrStructAccess(TO_RETURN, i(), j()), dotArrStructAccess(SUB_ARR, i(), j()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
	...
	private void addPermutateOneFunction() {
		...
		code.add(
				voteStruct + space() + functionCode(PERMUTATE_ONE, voteStruct + space() + VOTES, unsignedIntVar(LENGTH))
						+ space() + CCodeHelper.OPENING_BRACES);
		code.add(voteStruct + space() + SUB_ARR + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(space() + unsignedIntVar(arrAccess(ALREADY_USED_ARR, V)) + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V) + space()
				+ lineComment(COMMENT_SET_BEGINNING));
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(dotArrStructAccess(SUB_ARR, i()), C) + CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), LENGTH));
		code.add(spaces(SPACES_PER_TAB) + uintVarEqualsCode(NEW_INDEX) + functionCode(NONDET_UINT)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB)
				+ functionCode(ASSUME,
						conjunct(parenthesize(leq(zero(), NEW_INDEX)), parenthesize(lt(NEW_INDEX, LENGTH))))
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), i()));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
				+ functionCode(ASSUME, neq(NEW_INDEX, arrAccess(ALREADY_USED_ARR, j()))) + CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(arrAccess(ALREADY_USED_ARR, i()), NEW_INDEX)
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB)
				+ varAssignCode(dotArrStructAccess(SUB_ARR, NEW_INDEX), dotArrStructAccess(VOTES, i()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(voteStruct + space() + TO_RETURN + CCodeHelper.SEMICOLON);
		...
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V));
		code.add(spaces(SPACES_PER_TAB)
				+ varAssignCode(dotArrStructAccess(TO_RETURN, i()), dotArrStructAccess(SUB_ARR, i()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.RETURN + space() + TO_RETURN + CCodeHelper.SEMICOLON);
	...
	private void addConcatTwoFunction() {
		...
		code.add(voteStruct + space()
				+ functionCode(CONCAT_TWO, voteStruct + space() + VOTES_ONE, unsignedIntVar(SIZE_ONE),
						voteStruct + space() + VOTES_TWO, unsignedIntVar(SIZE_TWO))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(voteStruct + space() + SUB_ARR + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V) + space()
				+ lineComment(COMMENT_SET_BEGINNING));
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + varAssignCode(dotArrStructAccess(SUB_ARR, i(), j()), C)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), SIZE_ONE) + space()
				+ lineComment(COMMENT_LIMIT_UPPER_BOUND));
		addVoteLoopBound(CONCAT_TWO + ".3");
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + functionCode(CCodeHelper.IF, lt(i(), V)) + space()
				+ CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_TWO_TABS)
				+ varAssignCode(dotArrStructAccess(SUB_ARR, i(), j()), dotArrStructAccess(VOTES_ONE, i(), j()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), SIZE_TWO) + space()
				+ lineComment(COMMENT_LIMIT_UPPER_BOUND));
		addVoteLoopBound(CONCAT_TWO + ".5");
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + functionCode(CCodeHelper.IF, lt(varAddCode(SIZE_TWO, i()), V))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_TWO_TABS)
				+ varAssignCode(dotArrStructAccess(SUB_ARR, i(), j()), dotArrStructAccess(VOTES_TWO, i(), j()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(electionDesc.getContainer().getInputStruct().getStructAccess() + space() + TO_RETURN
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V));
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(j(), lt(), C));
	...
	private void addConcatOneFunction() {
		...
		final String voteStruct = electionDesc.getContainer().getInputStruct().getStructAccess();
		code.add(voteStruct + space()
				+ functionCode(CONCAT_ONE, voteStruct + space() + VOTES_ONE, unsignedIntVar(SIZE_ONE),
						voteStruct + space() + VOTES_TWO, unsignedIntVar(SIZE_TWO))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(voteStruct + space() + SUB_ARR + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		...
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), SIZE_ONE) + space()
				+ lineComment(COMMENT_LIMIT_UPPER_BOUND));
		code.add(spaces(SPACES_PER_TAB) + functionCode(CCodeHelper.IF, lt(i(), V)) + space()
				+ CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
				+ varAssignCode(dotArrStructAccess(SUB_ARR, i()), dotArrStructAccess(VOTES_ONE, i()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), SIZE_TWO) + space()
				+ lineComment(COMMENT_LIMIT_UPPER_BOUND));
		code.add(spaces(SPACES_PER_TAB) + functionCode(CCodeHelper.IF, lt(varAddCode(SIZE_ONE, i()), V)) + space()
				+ CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
		...
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V));
		code.add(spaces(SPACES_PER_TAB) + eq(dotArrStructAccess(TO_RETURN, i()), dotArrStructAccess(SUB_ARR, i()))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.RETURN + space() + TO_RETURN + CCodeHelper.SEMICOLON);
	...
	private void addSplitArrayFunctions() {
		...
		code.add(unsignedIntVar(pointer(getRandomSplitLinesFunction(unsignedIntVar(SPLITS), unsignedIntVar(MAX))))
				+ space() + CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + uintVarEqualsCode(pointer(SPLIT_ARR))
				+ functionCode(MALLOC, varMultiplyCode(SPLITS, functionCode(CCodeHelper.SIZE_OF, pointer(SPLIT_ARR))))
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_HALF_TAB);
		code.add(spaces(SPACES_PER_HALF_TAB) + functionCode(CCodeHelper.IF, eq(SPLITS, one())) + space()
				+ CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_TAB) + uintVarEqualsCode(NEXT_SPLIT) + functionCode(NONDET_UINT)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + functionCode(ASSUME, leq(zero(), NEXT_SPLIT)) + CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + functionCode(ASSUME, leq(NEXT_SPLIT, parenthesize(varDivideCode(MAX, two()))))
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(arrAccess(SPLIT_ARR, zero()), NEXT_SPLIT)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES + space() + CCodeHelper.ELSE + space()
				+ CCodeHelper.OPENING_BRACES);
		code.add(spaces(SPACES_PER_TAB) + uintVarEqualsCode(LAST_SPLIT) + zero() + CCodeHelper.SEMICOLON);
		...
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + uintVarEqualsCode(NEXT_SPLIT) + functionCode(NONDET_UINT)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + functionCode(ASSUME, leq(LAST_SPLIT, NEXT_SPLIT))
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + functionCode(ASSUME, leq(NEXT_SPLIT, MAX))
				+ CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_ONE_AND_HALF_TABS);
		code.addSpaces(SPACES_PER_ONE_AND_HALF_TABS);
		code.addSpaces(SPACES_PER_ONE_AND_HALF_TABS);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + varAssignCode(arrAccess(SPLIT_ARR, i()), NEXT_SPLIT)
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + varAssignCode(LAST_SPLIT, NEXT_SPLIT) + CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + unsignedIntVar(pointer(SPLIT_LINES)) + CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + varAssignCode(SPLIT_LINES, SPLIT_ARR) + CCodeHelper.SEMICOLON);
		code.addSpaces(SPACES_PER_TAB);
		code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(i(), lt(), SPLITS));
		addVoteLoopBound(GET_RANDOM_SPLIT_LINES + ".1");
		code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + uintVarEqualsCode("debugrandom") + arrAccess(SPLIT_LINES, i())
				+ CCodeHelper.SEMICOLON);
		code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
		code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.RETURN + space() + SPLIT_ARR + CCodeHelper.SEMICOLON);
		code.add(CCodeHelper.CLOSING_BRACES);
		code.add();
		...
		if (electionDesc.getContainer().getInputType().getAmountOfDimensions() == 1
				&& electionDesc.getContainer().getInputType().getSizeOfDimensions()[0].equals(V)) {
			code.add(voteStruct + space()
					+ functionCode(SPLIT, voteStruct + space() + VOTES, unsignedIntVar(START), unsignedIntVar(STOP))
					+ space() + CCodeHelper.OPENING_BRACES);
			code.add(voteStruct + space() + SUB_ARR + CCodeHelper.SEMICOLON);
			code.addSpaces(SPACES_PER_HALF_TAB);
			code.add(spaces(SPACES_PER_HALF_TAB) + forLoopHeaderCode(i(), lt(), V) + space()
					+ lineComment(COMMENT_SET_BEGINNING));
			code.add(spaces(SPACES_PER_TAB) + varAssignCode(dotArrStructAccess(SUB_ARR, i()), C)
					+ CCodeHelper.SEMICOLON);
			code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES);
			code.addSpaces(SPACES_PER_HALF_TAB);
			code.add(spaces(SPACES_PER_HALF_TAB) + functionCode(CCodeHelper.IF, eq(START, STOP)) + space()
					+ CCodeHelper.OPENING_BRACES + space() + lineComment(COMMENT_EMPTY_SUB_ARRAY));
			code.add(electionDesc.getContainer().getInputStruct().getStructAccess() + space() + TO_RETURN
					+ CCodeHelper.SEMICOLON);
			code.addSpaces(SPACES_PER_TAB);
			...
			code.add(spaces(SPACES_PER_TAB) + CCodeHelper.CLOSING_BRACES);
			code.addSpaces(SPACES_PER_TAB);
			code.add(spaces(SPACES_PER_TAB) + CCodeHelper.RETURN + space() + TO_RETURN + CCodeHelper.SEMICOLON);
			code.add(spaces(SPACES_PER_HALF_TAB) + CCodeHelper.CLOSING_BRACES + space() + CCodeHelper.ELSE + space()
					+ CCodeHelper.OPENING_BRACES);
			code.add();
			code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(i(), lt(), V));
			code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
					+ functionCode(CCodeHelper.IF, conjunct(parenthesize(leq(START, i())), parenthesize(lt(i(), STOP))))
					+ space() + CCodeHelper.OPENING_BRACES);
			code.add(spaces(SPACES_PER_TWO_TABS)
		...
		if (electionDesc.getContainer().getInputType().getAmountOfDimensions() == SPACES_PER_HALF_TAB
			...
			code.addSpaces(SPACES_PER_TAB);
			code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(i(), lt(), V));
			code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + forLoopHeaderCode(j(), lt(), V));
			code.add(spaces(SPACES_PER_TWO_TABS)
					+ varAssignCode(dotArrStructAccess(TO_RETURN, i(), j()), dotArrStructAccess(SUB_ARR, i(), j()))
					+ CCodeHelper.SEMICOLON);
			code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS) + CCodeHelper.CLOSING_BRACES);
			...
			code.add(spaces(SPACES_PER_TAB) + forLoopHeaderCode(i(), lt(), V));
			code.add(spaces(SPACES_PER_ONE_AND_HALF_TABS)
					+ functionCode(CCodeHelper.IF, conjunct(leq(START, i()), lt(i(), STOP))) + space()
					+ CCodeHelper.OPENING_BRACES);
			code.add(spaces(SPACES_PER_TWO_TABS) + forLoopHeaderCode(j(), lt(), C));
			code.add(spaces(SPACES_PER_TWO_AND_HALF_TABS)
					+ varAssignCode(dotArrStructAccess(SUB_ARR, varSubtractCode(i(), START), j()),
	 ...
	 *
	 * @param codeLst the list in which the header should be written
	...
	public CodeArrayListBeautifier addHeader(final CodeArrayListBeautifier codeLst) {
		...
		codeLst.add();
		codeLst.add(unsignedIntVar(functionCode(NONDET_UINT)) + CCodeHelper.SEMICOLON);
		codeLst.add(intVar(functionCode(NONDET_INT)) + CCodeHelper.SEMICOLON);
		codeLst.add();
		codeLst.add(define(functionCode(ASSERT2, x(), y()), functionCode(CPROVER_ASSERT, x(), y())));
		codeLst.add(define(functionCode(ASSUME, x()), functionCode(CPROVER_ASSUME, x())));
		codeLst.add();
		codeLst.addAll(Arrays.asList(electionDesc.getContainer().getStructDefinitions().split("\\n")));
		return codeLst;
	}
	...
	private void addMainMethod() {
		code.add(intVar(functionCode(MAIN, intVar("argc"), CCodeHelper.CHAR + space() + pointer("argv[]"))) + space()
				+ CCodeHelper.OPENING_BRACES);
		code.addTab();
		// Generating the pre and post AbstractSyntaxTrees.
		final BooleanExpListNode preAST = generateAST(preAndPostCondDesc.getPreConditionsDescription().getCode());
		final BooleanExpListNode postAST = generateAST(preAndPostCondDesc.getPostConditionsDescription().getCode());
		...
		for (int i = 1; i <= numberOfTimesVoted; i++) {
			code.add(uintVarEqualsCode(UnifiedNameContainer.getVoter() + i) + UnifiedNameContainer.getVoter()
					+ CCodeHelper.SEMICOLON);
		}
		// Initialize all voting variables for the candidates.
		for (int i = 1; i <= numberOfTimesVoted; i++) {
			code.add(uintVarEqualsCode(UnifiedNameContainer.getCandidate() + i) + UnifiedNameContainer.getCandidate()
					+ CCodeHelper.SEMICOLON);
		}
		// Initialize all voting variables for the seats.
		for (int i = 1; i <= numberOfTimesVoted; i++) {
			code.add(uintVarEqualsCode(UnifiedNameContainer.getSeats() + i) + UnifiedNameContainer.getSeats()
					+ CCodeHelper.SEMICOLON);
		}
		final List<String> boundedVars = preAndPostCondDesc.getBoundedVarDescription().getCodeAsList();
		...
		for (int voteNumber = 1; voteNumber <= numberOfTimesVoted; voteNumber++) {
			...
			addInitialisedValue(UnifiedNameContainer.getVotingArray() + voteNumber,
		...
		for (int i = 1; i <= numberOfTimesVoted; i++) {
			...
			final String electX = varAssignCode(
					electionDesc.getContainer().getOutputStruct().getStructAccess() + space() + ELECT + i,
					functionCode(UnifiedNameContainer.getVotingMethod(), sizeOfVotes, VOTES + i))
					+ CCodeHelper.SEMICOLON;
			code.add(electX);
		}
	...
	private void addSymbVarInitialisation() {
		final List<SymbolicVariable> symbolicVariableList = preAndPostCondDesc.getSymbolicVariablesAsList();
		code.add(lineComment("Symbolic variables initialisation"));
		symbolicVariableList.forEach(symbVar -> {
			final InternalTypeContainer internalType = symbVar.getInternalTypeContainer();
			final String id = symbVar.getId();
			if (!internalType.isList()) {
				switch (internalType.getInternalType()) {
				case VOTER:
					code.add(uintVarEqualsCode(id) + functionCode(NONDET_UINT) + CCodeHelper.SEMICOLON);
					...
					code.add(functionCode(ASSUME, conjunct(leq(zero(), id), lt(id, V))) + CCodeHelper.SEMICOLON);
				...
				case CANDIDATE:
					...
					// 0 and so on.
					code.add(functionCode(ASSUME, conjunct(leq(zero(), id), lt(id, C))) + CCodeHelper.SEMICOLON);
				...
				case SEAT:
					...
					code.add(functionCode(ASSUME, conjunct(leq(zero(), id), lt(id, S))) + CCodeHelper.SEMICOLON);
					break;
				case APPROVAL:
					break;
	...
	private void reportUnsupportedType(final String id) {
		ErrorLogger.log("The type of the symbolic variable " + id + " is not supported.");
	}
	 ...
	 * @param postAST the post AST
	 */
	private void initializeNumberOfTimesVoted(final BooleanExpListNode preAST, final BooleanExpListNode postAST) {
	 ...
	 */
	private void addInitialisedValue(final String valueName, final InOutType inOutType, final ComplexType complexType,
			final String minValue, final String maxValue) {
		code.add(lineComment("Init of variable" + space() + valueName));
		final String declaration = complexType.getStructAccess() + space() + valueName + CCodeHelper.SEMICOLON;
		code.add(declaration);
		final List<String> loopVariables = addNestedForLoopTop(this.code, inOutType.getSizeOfDimensionsAsList(),
				new ArrayList<String>());
		String assignment = valueName + dot() + UnifiedNameContainer.getStructValueName();
		for (int i = 0; i < inOutType.getAmountOfDimensions(); i++) {
			assignment += arrAcc(loopVariables.get(i));
		}
		code.add(varAssignCode(assignment, functionCode(NONDET_UINT)) + CCodeHelper.SEMICOLON);
		code.add(functionCode(ASSUME,
				conjunct(parenthesize(leq(minValue, assignment)), parenthesize(leq(assignment, maxValue))))
				+ CCodeHelper.SEMICOLON);
	 ...
	 * @return the list
	 */
	private List<String> generateLoopVariables(final int dimensions, final String variableName) {
	 ...
	 * @return the string
	 */
	private String generateForLoopHeader(final String indexName, final String maxSize) {
