...
public class InOutTypeAdapter implements JsonSerializer<InOutType>, JsonDeserializer<InOutType> {
	...
	@Override
	public InOutType deserialize(JsonElement json, Type typeOf, JsonDeserializationContext context)
			throws JsonParseException {
		JsonObject jsonObject = json.getAsJsonObject();
		String type = jsonObject.get("type").getAsString();
		JsonElement element = jsonObject.get("properties");
		try {
			for (Iterator<InputType> iterator = InputType.getInputTypes().iterator(); iterator.hasNext();) {
			...
			for (Iterator<OutputType> iterator = OutputType.getOutputTypes().iterator(); iterator.hasNext();) {
				...
				if (outType.getClass().getSimpleName().equals(type)) {
					return context.deserialize(element, Class.forName(outType.getClass().getName()));
				}
			}
			return null;
	...
	@Override
	public JsonElement serialize(InOutType src, Type typeOfSrc, JsonSerializationContext context) {
