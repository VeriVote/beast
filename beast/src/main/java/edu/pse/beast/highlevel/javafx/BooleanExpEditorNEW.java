...
public final class BooleanExpEditorNEW implements MenuBarInterface {
	 ...
	 *
	 * @param prePropertyArea    the pre property area
	 * @param postPropertyArea   the post property area
	 * @param boundedVarCodeArea the bounded var code area
	 * @param propDesc           the prop desc
	 * @param currentTreeItem    the current tree item
	 */
	public BooleanExpEditorNEW(final NewPropertyCodeArea prePropertyArea, final NewPropertyCodeArea postPropertyArea,
			final BoundedVarCodeArea boundedVarCodeArea, final PreAndPostConditionsDescription propDesc,
		...
		this.symbolicCandidateVariableList = GUIController.getController().getCandidateTreeItems().getChildren();
		this.symbolicSeatVariableList = GUIController.getController().getSeatTreeItems().getChildren();
	}
	 ...
	 * @param fromExisting the from existing
	 * @return the string
	 */
	public synchronized String addSymbVar(final InternalTypeContainer container, final String toAdd,
	 ...
	 *
	 * @param propertyItem the property item
	...
	public void bringToFront() {
		...
		if (currentItem != null) {
			...
			BooleanExpEditorGeneralErrorFinder.hasErrors(currentItem,
					GUIController.getController().getElectionDescription().getContainer());
		}
	}
	...
	@Override
	public void open() {
		final PreAndPostConditionsDescription newDescription = convert(saverLoader.load());
	...
	public void save() {
		savePropertyTextAreasIntoDescription();
		final String json = propSaverLoader.createSaveString(currentPropertyDescription);
	 ...
	 * @param file        the file
	 */
	public void saveAs(final PreAndPostConditionsDescription description, final File file) {
		savePropertyTextAreasIntoDescription();
