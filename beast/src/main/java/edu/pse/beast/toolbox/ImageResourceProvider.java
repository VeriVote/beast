...
public class ImageResourceProvider {
    ...
    public static ImageResourceProvider getToolbarImages() {
        if (toolbar == null) {
            String[] ids = { "new", "load", "save", "save_as", "undo", "redo", "cut", "copy", "paste", "start", "stop",
                    "showHelpToUser", "simulation" };
            toolbar = new ImageResourceProvider("/core/images/toolbar/", ids);
        }
        return toolbar;
    }
    ...
    public ImageResourceProvider(String folder, String[] ids) {
        ...
        for (String s : this.ids) {
            ...
            File toRead = new File(SuperFolderFinder.getSuperFolder() + this.folder + s + ".png");

            // String location = folder + s + ".png";
            Image img = null;
            // img =
            // FileLoader.loadFileAsImage(getClass().getClassLoader().getResourceAsStream(location));
