...
public abstract class ToolbarHandler {
     ...
     *            the string resource
     * @param actionIdsAndListeners
     *            the action ids and listeners
     * @param jToolbar
     *            the j toolbar
     */
    public ToolbarHandler(final ImageResourceProvider imRes,
        ...
        for (int i = 0; i < actionIdsAndListeners.length; ++i) {
            ...
            currentButton.setIcon(
                    new ImageIcon(imageRes.getImageById(id).getScaledInstance(
     ...
     */
    private class ActionIdListenerSorter
            implements Comparator<ActionIdAndListener> {
