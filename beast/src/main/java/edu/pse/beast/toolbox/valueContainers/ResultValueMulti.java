...
public class ResultValueMulti implements ResultValue {
    ...
    List<ResultValue> values = new ArrayList<ResultValue>();
    ...
    public void addValueAtPos(List<Integer> indices, String value) {
        ...
        if (values.size() < index) { // extend the array if it is not big enough already
            if (indices.size() > 1) { // we have to insert another "Multi" value, as we have to save more than one
                                      // data point
        ...
        values.get(index).addValueAtPos(indices.subList(1, indices.size()), value); // insert the object
    }
