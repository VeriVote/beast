...
public class ElectionSimulationWindow extends JFrame implements ActionListener {
	 ...
	 * @param sli
	 */
	public ElectionSimulationWindow(StringLoaderInterface sli, ElectionTypeContainer container, ElectionSimulation controller, ElectionSimulationModel model) {
		this.model = model;
...
//		}
	...
	public void update() {
		...
		if (model.getAmountCandidates() > model.getFieldsPerWidth()) {
			horizontalScroll.setBounds((int) (model.getBorderMarginSmall() + (model.getElementWidth() * 3 - model.getScrollBarWidth() / 2)),
					model.getBorderMarginSmall() * 2,
					Math.min(model.getAmountCandidates() *  model.getWidthMultiplier() , (this.getWidth() - 2 * (model.getBorderMarginSmall() + (model.getElementWidth() * 3 - model.getScrollBarWidth() / 2)))),
					model.getScrollBarWidth());
			horizontalScroll.setValues(model.getHorizontalOffset(), 1, 0, (model.getAmountCandidates() - 1) * model.getElementWidth() * 2);
		...
		if (model.getAmountVoters() > model.getFieldsPerHeight()) {
			verticalScroll.setBounds((int) (model.getBorderMarginSmall() + (model.getElementWidth() * 2 - model.getScrollBarWidth() / 2)),
					model.getBorderMarginSmall() * 2 + model.getElementHeight(),
					model.getScrollBarWidth(),
					Math.min(model.getAmountVoters() * model.getHeightMultiplier(), (this.getHeight() - 2 * (model.getBorderMarginSmall() * 2 + model.getElementHeight()))));
			verticalScroll.setValues(model.getVerticalOffset(), 1, 0, (model.getAmountVoters() -1) * model.getElementHeight() * 2);
			this.add(verticalScroll);
		} else {
		    model.setVerticalOffset(0);
		}

		addCandidate.setBounds(((model.getAmountCandidates() + 2) *  model.getWidthMultiplier() - model.getHorizontalOffset()), model.getBorderMarginSmall(), model.getElementWidth(),
				model.getElementHeight());
		this.add(addCandidate);

		addVoter.setBounds(model.getBorderMarginSmall(),
				(int) ((model.getAmountVoters() + 1) * model.getHeightMultiplier() + model.getBorderMarginSmall() + model.getElementHeight()) - model.getVerticalOffset(), model.getElementWidth(),
				model.getElementHeight());
		this.add(addVoter);

		removeCandidate.setBounds(model.getBorderMarginSmall() + model.getElementWidth(), model.getBorderMarginSmall(), model.getElementWidth(), model.getElementHeight());
		this.add(removeCandidate);

		removeVoter.setBounds(model.getBorderMarginSmall(), model.getBorderMarginSmall() + model.getElementHeight(), model.getElementWidth(), model.getElementHeight());
		this.add(removeVoter);

		for (int i = 0; i < model.getAmountVoters(); i++) {
			model.getRows().get(i).setBounds((int) (model.getBorderMarginSmall() + model.getElementWidth() * 3),
					model.getBorderMarginSmall() + ((i + 1) * model.getHeightMultiplier() + model.getElementHeight() - model.getVerticalOffset()),
					 model.getWidthMultiplier() * model.getAmountCandidates(), model.getElementHeight());
			if(model.getRows().get(i).getY() >= model.getBorderMarginSmall() + model.getHeightMultiplier() + model.getElementHeight()) {
		...
		for (int i = 0; i < model.getVoters().size(); i++) {
			model.getVoters().get(i).setBounds(model.getBorderMarginSmall(),
					(int) ((i + 1) * model.getHeightMultiplier() + model.getBorderMarginSmall() + model.getElementHeight() - model.getVerticalOffset()), model.getElementWidth(),
					model.getElementHeight());
			if(model.getVoters().get(i).getY() >= (model.getHeightMultiplier() + model.getBorderMarginSmall() + model.getElementHeight())) {
				this.add(model.getVoters().get(i));				
			}
		}

		for (int i = 0; i < model.getCandidates().size(); i++) {
			model.getCandidates().get(i).setBounds(((i + 2) *  model.getWidthMultiplier()) - model.getHorizontalOffset(), model.getBorderMarginSmall(), model.getElementWidth(), model.getElementHeight());
...
//		if(amount > 0 ) {
//			verticalOffset = Math.min(verticalOffset + amount * 5,  (amountVoters - 1) * model.getElementHeight() * 2);
//			update();
//		} else {
