...
public class FormalExpErrorFinderTreeListener
	 ...
	 */
	public FormalExpErrorFinderTreeListener(SymbolicVariableList list, NewCodeArea codeArea, ElectionDescription elecDescription) {
	...
	public void exitComparisonExp(FormalPropertyDescriptionParser.ComparisonExpContext ctx) {
		...
		if (ctx.typeExp(1).getChild(0) instanceof IntersectExpContext) { //the right arguments
			rhs = new IntersectTypeExpNode(elecDescription.getContainer().getOutputType(), (IntersectExpContext) ctx.typeExp(1).getChild(0));
		} else {
			rhs = expStack.pop();
		}

		if (ctx.typeExp(0).getChild(0) instanceof IntersectExpContext) { //the left argument
			lhs = new IntersectTypeExpNode(elecDescription.getContainer().getOutputType(), (IntersectExpContext) ctx.typeExp(0).getChild(0));
		} else {
			lhs = expStack.pop();
		}
