...
public class UnprocessedCBMCResult extends CBMCResult {
    ...
    public CBMCResultWrapperMultiArray getNewVotesWrappedMulti() {
        ...
        if (getResult() != null && getElectionDescription() != null) {
            ...
            //in case of a single choice vote we have to wrap the result in a two dimensional result object
            if (isSingle) {
    ...
    public Long[] getNewResult() {
        ...
        if (getResult() != null && getElectionDescription() != null) {
            ...
            // it is voting for seats, and not for one candidate
            if (getElectionDescription().getOutputType().getResultTypeSeats()) { 
                return readOneDimVar("new_result", getResult()).get(0).getArray();
            } else {
                Long[] tmpArray = {readLongs("new_result", getResult()).get(0).getValue()};
                return tmpArray;
            }
