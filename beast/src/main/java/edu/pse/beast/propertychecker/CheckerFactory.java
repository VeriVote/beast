            ...
            for (Iterator<Integer> candidateIterator = paramSrc.getParameter().getAmountCandidates()
                ...
                for (Iterator<Integer> seatsIterator = paramSrc.getParameter().getAmountSeats()
                    ...
                    synchronized (this) {
                        if (!stopped) {
                            ...
                            if (!fromFile) {
                                ...
                                currentlyRunning = startProcess(toCheck, advanced, voters, candidates, seats, this);
                    ...
                    if (stopped) {
                        ...
                        if (!fromFile) { //if it got started normally
                            // the checker finished checking for these specific
                            // parameters without being stopped and without a
                            // failure
                            // from the outside
                            // set currentlyRunnign to null, so we can catch, if
                            // the
                        ...
                        } else { //if the check got started by giving a file
                            currentlyRunning = null;

                            // if the last check was successful, we have to keep
                            // checking the other ones
                            if (checkAssertionSuccess(lastResult) | checkAssertionFailure(lastResult)) {                          
                                finished = true;
                                result.setNumVoters(voters);
                                result.setNumCandidates(candidates);
                                result.setNumSeats(seats);
