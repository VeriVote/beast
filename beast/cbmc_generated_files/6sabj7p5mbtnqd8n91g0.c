...
{
    VoteResultStruct temp_intersection_1;
    ...
    for (int i = 0; i < result2.amtResult; ++i) {
        ...
        if(contains) {
            temp_intersection_1.result[count] = nondet_uint();
            assume(temp_intersection_1.result[count] == result2.result[i]);
            count++;
        }
    }
    temp_intersection_1.amtResult = nondet_uint();
    assume(temp_intersection_1.amtResult == count);
    for (int i = count; i < MAX_CANDIDATES; ++i) {
        temp_intersection_1.result[i] = nondet_uint();
        assume(temp_intersection_1.result[i] == INVALID_VOTE);
    }
    electIntersection0 = temp_intersection_1;
}
    ...
    VoteResultStruct electIntersection3;
{
    ...
    electIntersection3 = temp_intersection_1;
}
